...
/* Supabase is optional. When config.js has no URL/key (default for this
   build), the site runs fully on the local data in data.js. */
const sbClient = (function () {
  try {
...
/* ============================================================
   PROJECT GEMS (likes) — local-first (localStorage), optional server sync
============================================================ */
...
function gemSvg(filled) {
  // Gem icon. Unliked = outline gem, liked = solid gem (styled via CSS).
  const cls = filled ? "gem-icon gem-filled" : "gem-icon gem-outline";
  return `<svg class="${cls}" viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
    <path class="gem-body" d="M6.4 3h11.2l3.9 5.7L12 21.6 2.5 8.7 6.4 3z"/>
    <path class="gem-facets" d="M2.5 8.7h19M9.2 8.7L12 21.6l2.8-12.9M9.2 8.7L11.1 3M14.8 8.7L12.9 3" fill="none"/>
  </svg>`;
}

function likeLabel(liked) {
  return {
    en: liked ? "Remove gem" : "Give a gem",
    fr: liked ? "Retirer le gem" : "Donner un gem",
    ar: liked ? "إزالة الجوهرة" : "أعجبني",
  }[currentLang()] || (liked ? "Remove gem" : "Give a gem");
}

/* Local-first count: base count from data/DB + this visitor's own gem. */
function displayLikeCount(projectId, baseCount) {
  const base = Number(baseCount) || 0;
  return base + (hasLiked(projectId) ? 1 : 0);
}
...
function applyLikeVisual(btn, liked) {
  ...
  btn.title = label;
  const icon = btn.querySelector(".gem-icon");
  if (icon) {
    const wrap = document.createElement("div");
    wrap.innerHTML = gemSvg(liked);
...
function likeButtonHtml(projectId, count, sizeClass = "") {
  ...
  return `
    <button type="button" class="like-btn${liked ? " liked" : ""}${sizeClass ? " " + sizeClass : ""}"
      ...
      data-base-count="${escHtml(String(Number(count) || 0))}"
      aria-pressed="${liked ? "true" : "false"}"
      aria-label="${escHtml(label)}"
      title="${escHtml(label)}">
      ${gemSvg(liked)}
      <span class="like-count">${escHtml(formatLikeCount(displayLikeCount(projectId, count)))}</span>
    </button>
  `;
}

/* Sync every button on screen that points at the same project. */
function syncLikeButtons(idStr, liked) {
  document.querySelectorAll(`.like-btn[data-project-id="${CSS.escape(idStr)}"]`).forEach((b) => {
    applyLikeVisual(b, liked);
    const c = b.querySelector(".like-count");
    if (c) c.textContent = formatLikeCount(displayLikeCount(idStr, b.dataset.baseCount));
  });
}

/* Works with NO backend at all: the gem is stored in localStorage only.
   If Supabase is configured, we additionally sync the shared counter. */
async function toggleProjectLike(projectId, btn) {
  ...
  if (currentlyLiked) likedSet.delete(idStr);
  else likedSet.add(idStr);
  saveLikedSet(likedSet);

  syncLikeButtons(idStr, !currentlyLiked);
  ...
  // ---- optional server sync (skipped entirely in local mode) ----
  if (!sbClient) { btn.dataset.busy = "0"; return; }
  ...
  try {
    ...
    if (data && typeof data.likes_count === "number") {
      ...
      // server count already includes this visitor's row -> store it as base minus own gem
      const base = Math.max(0, data.likes_count - (getLikedSet().has(idStr) ? 1 : 0));
      if (proj) proj.likes_count = base;
      document.querySelectorAll(`.like-btn[data-project-id="${CSS.escape(idStr)}"]`)
        .forEach((b) => { b.dataset.baseCount = String(base); });
      syncLikeButtons(idStr, getLikedSet().has(idStr));
    }
  } catch (err) {
    console.warn("Gem sync failed (kept locally):", err);
  } finally {
    btn.dataset.busy = "0";
  }
}
...
function buildContactForm() {
  ...
  document.getElementById("contactForm").addEventListener("submit", async (e) => {
    ...
    const form = e.target;
    if (!sbClient) {
      // No database configured — hand the message over to WhatsApp
      const d = Object.fromEntries(new FormData(form).entries());
      const txt = [
        (d.name ? "Nom / الاسم: " + d.name : ""),
        (d.subject ? "Sujet / الموضوع: " + d.subject : ""),
        (d.email ? "Email: " + d.email : ""),
        "",
        d.message || "",
      ].filter(Boolean).join("\n");
      window.open("https://wa.me/" + AUTODEV_WHATSAPP + "?text=" + encodeURIComponent(txt), "_blank");
      const st = document.getElementById("contactFormStatus");
      if (st) st.textContent = pickI18n(contactFormI18n.success);
      form.reset();
      return;
    }
...
function applySeoMeta() {
  ...
  setMeta("property", "og:site_name", (site && site.site_name) || "AutoDev Agency");
...
async function initBackend() {
  buildContactForm();
  if (!sbClient) { console.info("AutoDev portfolio: running on local content (data.js)."); return; }
